...
namespace NetCryptoBench
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                ...
                flavor = "net10.0-OSX-Release-arm64";
            }
            ...
            config.AddCustom100Toolchain(
                displayName: "main",
                coreRunDirectory: Path.Join(basePath, "/runtime-main/artifacts/bin/testhost/", flavor, "/shared/Microsoft.NETCore.App/10.0.0/"),
                isBaseline: true);

            config.AddCustom100Toolchain(
                displayName: "branch",
                coreRunDirectory: Path.Join(basePath, "/runtime/artifacts/bin/testhost/", flavor, "/shared/Microsoft.NETCore.App/10.0.0/"));
    ...
    public class LocalCoreClrConfig : ManualConfig
    {
        ...
        // Thanks to https://github.com/GrabYourPitchforks/ConsoleApplicationBenchmark/blob/e0b0048198c856a30cacec19e3edc52c75d0677d/ConsoleAppBenchmark/Program.cs
        public void AddCustom100Toolchain(string displayName, string coreRunDirectory, bool enableTieredCompilation = true, bool isBaseline = false, Dictionary<string, string> envVars = default)
        {
            var toolchain = new CoreRunToolchain(
                coreRun: new DirectoryInfo(coreRunDirectory).GetFiles("corerun").Single(),
